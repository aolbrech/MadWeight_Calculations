///////////////////////////////////////////////////////
//    Performs polynomial fit on limited range       //
//    --> Was needed to cope with empty bins         //
//    --> Now superseeded by FitDeviationScript ...  //
//        (here TGraph is created!)                  //
///////////////////////////////////////////////////////
#include "TH1.h"
#include "TF1.h"
#include "TList.h"
...
void fitExcludeEmptyBins() {
  ...
  TFile* InputFile = new TFile(" Events/RVR_RecoCorrect_SingleGausTF_10000Evts_ManySteps/Likelihood_CorrectReco_RVR.root ","r");
  TFile* OutputFile = new TFile(" Events/RVR_RecoCorrect_SingleGausTF_10000Evts_ManySteps/LimitedFitResult_CorrectReco_RVR.root ","RECREATE");
  ...
  int nBins = 9;         //Initialize nrBins 
  float fitStart = -0.112777777778;  //Initialize fitStart 
  float fitEnd = 0.112777777778;    //Initialize fitEnd 
  ...
  for(int ii = 0; ii < nBins; ii++){
    LLFit->SetBinContent(ii+1,LL->GetBinContent(LL->FindBin(firstBin+ii*((fitEnd-fitStart)/nBins)))); 
    LLXSFit->SetBinContent(ii+1,LLXS->GetBinContent(LLXS->FindBin(firstBin+ii*((fitEnd-fitStart)/nBins)))); 
    if(GenLevel == false) LLAccFit->SetBinContent(ii+1,LLAcc->GetBinContent(LLAcc->FindBin(firstBin+ii*((fitEnd-fitStart)/nBins)))); 
  }
  LL->SetMarkerStyle(20);   LL->SetMarkerColor(1);     
  LLXS->SetMarkerStyle(21); LLXS->SetMarkerColor(1); 
  if(GenLevel == false){ LLAcc->SetMarkerStyle(22); LLAcc->SetMarkerColor(1);} 
  LLFit->Write(); LLXSFit->Write();
