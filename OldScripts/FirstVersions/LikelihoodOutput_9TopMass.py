...
MTopValues = ["m_{top} = 153",  "m_{top} = 163",  "m_{top} = 171",  "m_{top} = 172",  "m_{top} = 173",  "m_{top} = 174",  "m_{top} = 175","m_{top} = 183",  "m_{top} = 193"]
MTop =       [153,              163,              171,              172,              173,              174,              175,            183,              193            ]
MGXS =       [8.20916,          9.6299,           10.70485,         10.8257,          10.96469,         11.08428,         11.22448,       12.18068,         13.3046        ]
MGXSe =      [0.00641266950107, 0.00775899974932, 0.00814303595657, 0.00878899028501, 0.00816801717126, 0.00904797742371, 0.008653800078, 0.00931290317946, 0.0103310001752]
Acceptance = [0.16203,          0.19152,          0.21460,          0.21735,          0.21290,          0.21752,          0.22185,        0.23941,          0.26413        ]
...
for LLLine in LLFile:
  ...
  if str(LLWord[0]) != "#":
    if str(LLWord[1]) != "inf":
      ...
      LLNormDist.SetBinContent(LLDist.FindBin(MTop[int(LLWord[0])-1]),float(LLWord[1]) + int(NrEvents)*log(MGXS[int(LLWord[0])-1]))
